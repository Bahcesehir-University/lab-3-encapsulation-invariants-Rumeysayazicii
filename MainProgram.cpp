...
public:
    ...
    void setCelsius(double celsius) {
        ...
        if (celsius < -273.15) {
            throw invalid_argument("Temperature connot be below absolute zero");
        }
        celsius_ = celsius;
    }
};
...
public:
    ...
    BankAccount(const string& owner, double initialBalance) {
        ...
        if (initialBalance < 0) {
            throw invalid_argument( "Inıtial balance cannot be negative.");
        }
        owner_=owner;
...
private:
    ...
    static void validate(const string& pwd) {
        ...
        if( !hasDigit(pwd)) {
            throw invalid_argument("Password must be contain at least one digit");
        }
    }
